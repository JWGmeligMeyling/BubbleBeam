...
public class ColouredBubble extends AbstractBubble {
	...
	protected void renderDebugLines(final Graphics2D g2) {
		...
		if (this.hasRight()) {
			g2.drawLine(this.getCenter().x, this.getCenter().y, this.getRight()
					.getCenter().x, this.getRight().getCenter().y);
		}

		if (this.hasBottomRight()) {
			g2.drawLine(this.getCenter().x, this.getCenter().y, this
					.getBottomRight().getCenter().x, this.getBottomRight()
					.getCenter().y);
		}

		if (this.hasBottomLeft()) {
			g2.drawLine(this.getCenter().x, this.getCenter().y, this
					.getBottomLeft().getCenter().x, this.getBottomLeft()
					.getCenter().y);
		}
	}
